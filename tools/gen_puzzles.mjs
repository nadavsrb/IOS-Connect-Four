...
// winIn → tier name. Distinct all the way to 10, because that's exactly where
// the puzzles get hardest — lumping 7–10 under one name hid the ramp.
const TIER = {
  1: 'Warm-up', 2: 'Sharp', 3: 'Tactician', 4: 'Sniper', 5: 'Grandmaster',
  6: 'Legend', 7: 'Mastermind', 8: 'Oracle', 9: 'Nightmare', 10: 'Impossible',
};
const tierName = (winIn) => TIER[Math.min(10, winIn)];

// --- Preserve the committed set verbatim (stable ids ⇒ stable saved progress). ---
let base = [];
try {
  const mod = await import('../js/puzzles.js');
  base = mod.PUZZLES.map((p) => ({ id: p.id, grid: p.grid, line: p.line.slice(), winIn: p.winIn, discs: discsOf(p.grid) }));
} catch { base = []; }
...
// Emit one strictly non-decreasing ladder: deeper mate first, and within a band a
// fuller board first (tighter position). Carried-over puzzles keep their id so
// saved progress stays valid; only genuinely new ones get fresh ids.
let nextId = 1 + all.reduce((m, p) => Math.max(m, p.id || 0), 0);
for (const p of all) if (!p.id) p.id = nextId++;
all.sort((a, b) => (a.winIn - b.winIn) || (b.discs - a.discs) || (a.id - b.id));

const rows = all.map((p) =>
  `  { id: ${p.id}, tier: '${tierName(p.winIn)}', winIn: ${p.winIn}, grid: '${p.grid}', line: [${p.line.join(', ')}] },`
).join('\n');

const out = `// AUTO-GENERATED by tools/gen_puzzles.mjs — do not edit by hand.
//
// A progressive set of "you play and win" Connect Four puzzles: one ladder sorted
// strictly easiest → hardest (win in 1 through win in 10, Warm-up → Impossible).
// `id` is a permanent handle used by saved progress; the number shown to the
// player is the puzzle's position in this list.
